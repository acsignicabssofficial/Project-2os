 ...
 * for the "2OS" Accounting System pitch deck.
 * Faithfully mirrors the real 2OS application interface: Top Bar, Ribbon Tabs,
 * Grouped Tools, Export & Convert panel, Document Header Card, Action Bar, and Sheet Bar.
 */
export async function generate2OSPitchDeck(options: GeneratePptxOptions = {}): Promise<void> {
  const pptx = new PptxGenJS();

  // Explicit Widescreen 16:9 definition (13.333 x 7.5 inches)
  pptx.defineLayout({ name: 'WIDESCREEN_16x9', width: 13.333, height: 7.5 });
  pptx.layout = 'WIDESCREEN_16x9';

  pptx.author = options.author || '2OS Founder & Executive Team';
  pptx.company = options.companyName || '2OS Accounting System';
  ...
  function addTopBanner(slide: PptxGenJS.Slide) {
  ...
  function addSlideHeading(slide: PptxGenJS.Slide, badge: string, title: string, subtitle?: string) {
    addTopBanner(slide);
  ...
  interface FrameOptions {
    ...
    categoryTools?: { key: string; label: string; icon: string }[];
    categoryToolsGroupLabel?: string;
  ...
  // Build the 11 Slides
  options.onProgress?.('Generating Slide 1: Cover & Creed...');
  // 1. Cover
  {
  ...
  // 2. Anatomy
  options.onProgress?.('Generating Slide 2: UI Anatomy...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 3. Home Ribbon
  options.onProgress?.('Generating Slide 3: Home Ribbon...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 4. Directory Ribbon
  options.onProgress?.('Generating Slide 4: Directory Ribbon...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 5. Books of Accounts Ribbon
  options.onProgress?.('Generating Slide 5: Books of Accounts...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 6. Other Transactions Ribbon (EXACT APP SCREENSHOT MATCH)
  options.onProgress?.('Generating Slide 6: Other Transactions / Payroll...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
    ...
    // Card Header Row: Left $ icon + Title + Subtitle
    slide.addText('$', {
      x: layout.contentX + 0.3,
  ...
  options.onProgress?.('Generating Slide 7: Financial Statements...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 8. BIR Computations
  options.onProgress?.('Generating Slide 8: BIR Computations...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 9. Matrix
  options.onProgress?.('Generating Slide 9: Competitive Matrix...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 10. Tiers
  options.onProgress?.('Generating Slide 10: Scalability Tiers...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
  ...
  // 11. Creed
  options.onProgress?.('Generating Slide 11: Creed & Next Steps...');
  {
    const slide = pptx.addSlide();
    slide.background = { color: C.canvasBg };
    addTopBanner(slide);
  ...
  options.onProgress?.('Finalizing PPTX presentation...');
  await pptx.writeFile({ fileName: '2OS_Accounting_Pitch_Deck.pptx' });
}
